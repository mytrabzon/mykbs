...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await req.json().catch(() => ({})) as { misafirId?: string };
    const id = body.misafirId;
    if (!id) return new Response(JSON.stringify({ message: "misafirId gerekli" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    const del = await supabase.from("guests").delete().eq("id", id).eq("branch_id", branchId);
